...
| 交互 | 行为 |
|---|---|
| 左栏型号名 | 主名用**唯一型号 id**（`STM32C011F4Px`），副行 `封装 · 脚数 · Flash`。数据手册式合并名（`displayName`，如 `STM32C011F(4-6)Px`）放 `title` 提示与详情卡——全库 2781 条里有 658 条按 displayName 显示会完全重名（同封装同脚数同 Flash） |
| 点击引脚 | 右栏显示详情；该引脚 `ring-2 ring-ring`；键盘可聚焦 |
| **功能类别过滤** | 图**上方**一排 chip：`全部 / GND / 5V / 3V3 / GPIO / UART / I2C / QSPI / I2S / JTAG / PDM / PWM`。点一下高亮命中引脚（描边加粗、其余 `opacity-25`），再点取消；chip 上带匹配数，**计数为 0 时禁用（不隐藏）**。与下方"引脚类型图例"互斥（点一个会清掉另一个），避免两套淡化同时作用 |
...
- **字号不写死**：由 `app/utils/label-policy.ts` 反推 —— 引脚号 `0.52×行距`、pad 名 `min(0.44×行距, (块宽−4)/1.15)`、球号 `0.44×格子`，上限 26/22/20，下限 9/9/10。
...
- **引脚号在块外侧、pad 名写在引脚块里**（CubeMX 风格）：这样"物理编号"与"GPIO 名"依然分得开，而且四角不再有"左右文字 vs 上下文字"的互侵（各自待在自己的块里）。
  - 左右两列是横长条 → 名字横排，左块左对齐、右块右对齐，基线落在 `cy + 0.35×字号`；
  - 上下两排是竖长条 → 名字竖排（`rotate(-90)` + `text-anchor=middle`），基线落在 `cx + 0.35×字号`。
    这两条偏移都是为了让**字身居中在块的中线**上；符号写反字就会偏出块（踩过，见 `padLabelBox()` 注释）。
  - 引脚号与同一条边的名字同向（上下两排也竖排），始终在块外侧 10 单位。
- pad 名超宽用 `fitText()` 截断加省略号（可用长度 = 块长 − 两侧内边距 12）。
- 引脚矩形圆角 `radius-sm`；`nc` 类型虚线描边；图例过滤时其余类型 `opacity-25`。
- 标注：quad/dual 在块外侧标 `position`、块内标 `pad`；grid 在球心标 `A1/B7`（球里放不下名字，靠 hover）。
- **本体中心印"丝印"**：`chip` + `封装 · N 脚` 两行（实物芯片顶面就是这么印的）。字号按型号长度反推（上限 44），
  颜色用 `muted-foreground` 免得抢引脚名；网格封装不放（球阵会盖住）。
- Pin 1 标记：左上角一个实心小圆（`r=11`，内缩 34）；**方向必须与数据手册一致**，见 05 §4。
