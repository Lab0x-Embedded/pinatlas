...
- **字号不写死**：由 `app/utils/label-policy.ts` 按行距/格子反推（引脚号 0.52×行距、pad 名 0.44×行距、球号 0.44×格子，上限 26/22/20，下限 9/9/10）。
- **Density Mode（按行距自动降级，不写具体型号）**：

  | 行距 / 格子 | 画什么 | 例子 |
  |---|---|---|
  | ≥ 26 单位 | 引脚号 + 主名 | LQFP48（行距 48.3） |
  | 14 ~ 26 | 只画引脚号 | LQFP100（行距 23.2） |
  | < 14 | 只画引脚块（连号也不画） | LQFP208（每边 52 脚 → 行距 11.2） |
  | 网格 < 26 | 只画行列坐标头，不写球号 | TFBGA361（格子 18.1） |

  名称始终可从 hover 提示与右侧 Inspector 读到。
- 图上只画 `primary` 主名（v1.1.0），别名/重映射标注进 hover 与 Inspector。
